...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.layer5(x)
        #print(x.shape)
        x_up = F.interpolate(x,scale_factor=8,mode='bilinear')
        #print(x.shape)
...
def deeplab_res50(num_class = 5):
    return ResNet(Bottleneck , [3, 4, 6, 3], num_class)


def deeplab_res101(num_class = 5):
...
if __name__ == '__main__':
    ...
    for i in tqdm(range(1)):
        img = torch.rand(32, 3, 256, 128).cuda()  # 416 320     800 608
        out = model(img)
    print(out[0].size(),out[1].size())

    map = out[1]
    v = F.adaptive_max_pool2d(map, 1)
    v = v.view(v.size(0), -1)
    print(v.shape)
    t2 = time.time()
    print(1000 / (t2 - t1)  )